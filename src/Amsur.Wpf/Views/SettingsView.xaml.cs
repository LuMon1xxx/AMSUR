...
// P-D3: настройки как view (логика из SettingsWindow 1-в-1).
...
public partial class SettingsView : UserControl
{
    ...
    private async void OnDangerToggleChanged(object sender, RoutedEventArgs e)
    {
        ...
        if (box.IsChecked == true)
        {
            bool ok = await _session.ConfirmDangerousAsync(
                System.Windows.Window.GetWindow(this), code, o.Title, DangerousConsequence(code));
            if (!ok)
            {
    ...
    private async void OnOverloadApplyClick(object sender, RoutedEventArgs e)
    {
        ...
        if (raising)
        {
            bool ok = await _session.ConfirmDangerousAsync(System.Windows.Window.GetWindow(this), "teacher-overload",
    ...
    // В однооконной навигации «Отмена» = выйти без применения:
    // view пересоздаётся при каждом входе, неприменённое исчезает само.
    private void OnCancelClick(object sender, RoutedEventArgs e) => Navigator?.NavigateTo("Dashboard");
    ...
    private async Task<bool> ConfirmCapRaisesAsync()
    {
        ...
        return await _session.ConfirmDangerousAsync(System.Windows.Window.GetWindow(this), "cap-raise", "Повышение дневных лимитов",
    ...
    private async void OnImportJsonClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            catch (InvalidOperationException) when (ov.Keys.Any(RuleCatalog.IsDangerous))
            {
                ...
                bool ok = await _session.ConfirmDangerousAsync(
                    System.Windows.Window.GetWindow(this), first, oh.Title,
